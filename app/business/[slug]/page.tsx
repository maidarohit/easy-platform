...
import { publicCallToAction, publicServices, publicServicesSummary } from "@/app/lib/public-business-presentation";
...
export default async function PublicBusinessPage({ params }: Props) {
  ...
  const services = publicServices(snapshot);
  const servicesSummary = publicServicesSummary(snapshot);
  const callToAction = publicCallToAction(snapshot);
  return <main className="min-h-screen bg-[#FCFBF7] text-[#1B211E]">
    <header className="border-b border-black/10 bg-white px-5 py-5 sm:px-10"><div className="mx-auto flex max-w-6xl items-center justify-between gap-5"><strong className="text-xl" style={{ color: primary }}>{snapshot.business.name}</strong><a href="#contact" className="rounded-full px-5 py-3 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2" style={{ backgroundColor: primary }}>{callToAction}</a></div></header>
    <section className="px-5 py-20 text-center text-white sm:px-10 sm:py-28" style={{ backgroundColor: primary }}><div className="mx-auto max-w-5xl"><p className="text-sm font-semibold uppercase tracking-[0.2em] opacity-65">{snapshot.business.industry}</p><h1 className="mt-5 text-4xl font-semibold leading-tight tracking-[-0.05em] sm:text-7xl">{website?.heroHeadline || snapshot.brand?.tagline || snapshot.business.name}</h1>{website?.supportingText && <p className="mx-auto mt-6 max-w-3xl text-lg leading-8 opacity-80">{website.supportingText}</p>}<a href="#contact" className="mt-8 inline-flex rounded-full bg-white px-7 py-4 font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-4" style={{ color: primary }}>{callToAction}</a></div></section>
    {(services.length > 0 || servicesSummary) && <section className="px-5 py-16 sm:px-10"><div className="mx-auto max-w-6xl"><p className="text-xs font-semibold uppercase tracking-[0.2em] opacity-60">What we offer</p>{services.length > 0 ? <div className="mt-7 grid gap-5 md:grid-cols-3">{services.map((service) => <article key={service.title} className="rounded-3xl border border-black/10 bg-white p-6"><h2 className="text-xl font-semibold" style={{ color: primary }}>{service.title}</h2>{service.description && <p className="mt-3 leading-7 opacity-70">{service.description}</p>}</article>)}</div> : <p className="mt-7 max-w-4xl whitespace-pre-wrap leading-8 opacity-75">{servicesSummary}</p>}</div></section>}
    ...
    <section id="contact" className="px-5 py-20 text-center text-white sm:px-10" style={{ backgroundColor: primary }}><h2 className="text-3xl font-semibold sm:text-5xl">Ready to take the next step?</h2><a href={website?.contact?.includes("@") ? `mailto:${website.contact}` : "#contact"} className="mt-7 inline-flex rounded-full bg-white px-7 py-4 font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-4" style={{ color: primary }}>{callToAction}</a>{website?.contact && <p className="mt-5 font-semibold">{website.contact}</p>}</section>
