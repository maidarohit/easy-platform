...
import {
  ...
  publicSeoTitle,
  publicServices,
  publicServicesSummary,
  publicStory,
  publicValuePoints,
  uniquePublicNavigationItems,
} from "@/app/lib/public-business-presentation";
...
function joinReadableServices(services: readonly { title: string }[]) {
  if (services.length === 0) return "";
  if (services.length === 1) return services[0]!.title;
  if (services.length === 2) return `${services[0]!.title} and ${services[1]!.title}`;
  return `${services[0]!.title}, ${services[1]!.title}, and ${services[2]!.title}`;
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = pageSeo?.title || publicSeoTitle(publicSnapshot);
  const description = pageSeo?.description || publicSeoDescription(publicSnapshot);
...
export default async function PublicBusinessPage({ params, searchParams }: Props) {
  ...
  const catalogue = await db.select({
    ...
    kind: projectProducts.kind,
    category: projectProducts.category,
  ...
  const selectedProductId = typeof requestedProduct === "string" && catalogue.some((item) => item.id === requestedProduct) ? requestedProduct : "";

  if (snapshot.siteDocument) {
    return <><WebsiteTrafficTracker kind="business" publicationId={published.publicationId} slug={slug} pagePath="/" /><WebsiteSiteRenderer document={snapshot.siteDocument} pagePath="/" basePath={`/business/${encodeURIComponent(slug)}`} industry={snapshot.business.industry ?? ""} description={snapshot.business.description ?? ""} media={{ hero: snapshot.website?.heroImage, work: snapshot.website?.secondaryImage }} serviceItems={publicServices(snapshot).map((service, index) => ({ id: `published-service-${index + 1}`, title: service.title, description: service.description }))} catalogueItems={catalogue} selectedProductId={selectedProductId} contact={snapshot.contact} inquirySlug={slug} checkoutReady={checkoutReady} publicPageOnly /></>;
  }

  const publicSnapshot = publicBusinessView(snapshot);
  const storeLabel = catalogueLabel(catalogue);
  const primary = publicSnapshot.brand?.colours[0] || "#173D32";
  const accent = publicSnapshot.brand?.colours[1] || "#E9E4D8";
  const cta = publicCallToAction(publicSnapshot);
  const contact = publicContact(publicSnapshot);
  const services = publicServices(publicSnapshot);
  const summary = publicServicesSummary(publicSnapshot);
  const process = publicProcess(publicSnapshot);
  const values = publicValuePoints(publicSnapshot);
  const story = publicStory(publicSnapshot);
  const kind = publicBusinessKind(publicSnapshot);
  const heroCopy = publicHeroCopy(publicSnapshot);
  const offeringLabel = kind.workLabel === "Featured range" ? "Products" : "Services";
  const businessName = publicSnapshot.business.name;
  const serviceSummary = joinReadableServices(services.slice(0, 3));
  const servicesHeading = `What ${businessName} offers`;
  const workHeading = `See more from ${businessName}.`;
  const valueHeading = `Why choose ${businessName}`;
  const aboutHeading = `About ${businessName}`;
  const contactHeading = services[0] ? `Enquire about ${services[0].title}` : `Contact ${businessName}`;
  const contactBody = serviceSummary ? `Ask about ${serviceSummary}.` : `Contact ${businessName} to learn more.`;
  const requestedService = query.service;
  const selectedService = typeof requestedService === "string" && services.some((item) => item.title === requestedService) ? requestedService : "";
  ...
  const media = resolveWebsiteMedia({
    ...
    uploaded: {
      ...
      work: uploadedSrcFromRecord(publicSnapshot.website, "showcase"),
    },
  });
