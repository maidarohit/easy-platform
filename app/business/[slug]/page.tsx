...
export default async function PublicBusinessPage({ params, searchParams }: Props) {
  ...
  const nav = uniquePublicNavigationItems([{ href: "#home", label: "Home" }, ...(services.length || summary ? [{ href: "#services", label: offeringLabel }] : []), ...(hasWork ? [{ href: "#work", label: kind.workLabel }] : []), ...(story ? [{ href: "#about", label: "About" }] : []), { href: "#process", label: "Process" }, ...(catalogue.length > 0 ? [{ href: "#store", label: storeLabel }] : []), { href: "#contact", label: "Contact" }]);
