...
export default async function PublicBusinessPage({ params, searchParams }: Props) {
  ...
  const summary = publicServicesSummary(snapshot); const process = publicProcess(snapshot);
  ...
  const media = resolveWebsiteMedia({ ...visualContext, uploaded: {
    hero: uploadedSrcFromRecord(snapshot.website, "hero"),
    work: uploadedSrcFromRecord(snapshot.website, "showcase"),
  } });
