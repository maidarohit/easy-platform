...
export default function RealEstateTemplate({
  ...
  const properties = [
    { name: `${businessName} 01`, detail: description.slice(0, 140), marker: "01", scene: "villa" },
    { name: `${businessName} 02`, detail: brandResult?.websiteFeatures || description, marker: "02", scene: "interior" },
    { name: `${businessName} 03`, detail: brandResult?.designRecommendations || description, marker: "03", scene: "garden" },
  ];
    ...
    <div className="overflow-hidden bg-[#0b0b0a] text-[#f3efe6]">
      ...
      <section className={compactPreview ? "relative overflow-hidden px-7 py-16" : "relative min-h-[760px] overflow-hidden px-12 py-28"}>
        ...
        <div className={compactPreview ? "relative z-10 mt-12 h-[420px] overflow-hidden border border-white/10 bg-[#191714] shadow-[0_35px_90px_rgba(0,0,0,0.55)]" : "absolute right-[6%] top-[9%] h-[68%] w-[46%] overflow-hidden border border-white/10 bg-[#191714] shadow-[0_50px_120px_rgba(0,0,0,0.55)]"}>
          ...
          <span className="absolute left-5 top-5 z-10 rounded-full border border-[#c9a35c]/30 bg-black/35 px-3 py-1.5 text-[9px] uppercase tracking-[0.2em] text-[#d8bd82] backdrop-blur">{labels.about}</span>
      ...
      <section className="border-y border-white/10 bg-[#f0eadf] px-7 py-20 text-[#17130e] lg:px-12">
        <div className={compactPreview ? "grid items-center gap-12" : "grid grid-cols-2 items-center gap-12"}><div className="relative min-h-96 overflow-hidden bg-gradient-to-br from-[#504532] via-[#29241c] to-[#13110e]"><span className="absolute left-5 top-5 z-10 rounded-full border border-[#e2c995]/35 bg-black/30 px-3 py-1.5 text-[9px] uppercase tracking-[0.2em] text-[#f0ddba] backdrop-blur">{labels.about}</span><div className="absolute left-[8%] top-[14%] h-[58%] w-[45%] border border-[#e1c58d]/35 bg-gradient-to-b from-[#8b7653]/35 to-[#211c16]"><div className="absolute inset-[12%] border border-[#efd9ac]/15"/></div><div className="absolute bottom-[13%] right-[7%] h-[34%] w-[48%] rounded-t-[48px] bg-[#d5c4a5]/28 shadow-[0_20px_50px_rgba(0,0,0,0.3)]"/><div className="absolute bottom-[8%] left-[5%] right-[5%] h-px bg-[#d3b77e]/45"/><div className="absolute bottom-[6%] right-[12%] h-16 w-16 rounded-full bg-[#555c43] shadow-[18px_-8px_0_#3d4632,-12px_-4px_0_#697054]"/></div><div><p className="text-[10px] uppercase tracking-[0.3em] text-[#8d6d35]">{labels.about} {businessName}</p><h2 className="mt-5 font-serif text-4xl leading-tight sm:text-5xl">{headline}</h2><p className="mt-6 whitespace-pre-wrap break-words leading-8 text-[#625a4f]">{brandResult?.designRecommendations || description}</p><div className="mt-8 grid grid-cols-2 gap-5">{[labels.services, labels.about, labels.howWeCanHelp, labels.contact].map(item => <div key={item} className="border-t border-[#aa9166] pt-4 text-sm font-medium">{item}</div>)}</div></div></div>
      </section>
