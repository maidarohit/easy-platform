...
const ACTION_BUTTON = "rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-xs font-semibold text-white shadow-sm transition-colors hover:border-cyan-500 hover:bg-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-100 disabled:cursor-not-allowed disabled:border-slate-300 disabled:bg-slate-200 disabled:text-slate-500 disabled:shadow-none";
const SAVE_BUTTON = "self-end rounded-lg border border-cyan-600 bg-cyan-900 px-3 py-2 text-xs font-semibold text-white shadow-sm transition-colors hover:border-cyan-500 hover:bg-cyan-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-100 disabled:cursor-not-allowed disabled:border-slate-300 disabled:bg-slate-200 disabled:text-slate-500 disabled:shadow-none";
const REMOVE_BUTTON = "rounded-lg border border-red-700 bg-red-900 px-3 py-2 text-xs font-semibold text-white shadow-sm transition-colors hover:border-red-600 hover:bg-red-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-100 disabled:cursor-not-allowed disabled:border-slate-300 disabled:bg-slate-200 disabled:text-slate-500 disabled:shadow-none";
...
function PageRow({ page, document, selected, disabled, onSelect, onSave }: { page: WebsitePage; document: WebsiteSiteDocument; selected: boolean; disabled: boolean; onSelect: (path: string) => void; onSave: (document: WebsiteSiteDocument) => Promise<boolean> }) {
  ...
  return <li className={`rounded-xl border bg-slate-100 p-4 text-slate-900 shadow-sm ${selected ? "border-cyan-500 ring-2 ring-cyan-500/30" : "border-slate-300"}`}>
    <div className="grid gap-3 lg:grid-cols-[1fr_1fr_auto]">
      ...
      <button type="button" disabled={disabled || (title === page.title && path === page.path)} onClick={saveRename} className={SAVE_BUTTON}>Save name/path</button>
    </div>
    <div className="mt-3 flex flex-wrap gap-2">
      <button type="button" onClick={() => onSelect(page.path)} disabled={disabled} className={ACTION_BUTTON}>Preview</button>
      <button type="button" onClick={() => move(-1)} disabled={disabled || index === 0} aria-label={`Move ${page.title} up`} className={ACTION_BUTTON}>Move up</button>
      <button type="button" onClick={() => move(1)} disabled={disabled || index === ordered.length - 1} aria-label={`Move ${page.title} down`} className={ACTION_BUTTON}>Move down</button>
      <button type="button" onClick={async () => { const next = setWebsitePageNavigationVisibility(document, page.id, !shownInNavigation); if (next) await onSave(next); }} disabled={disabled} className={ACTION_BUTTON}>{shownInNavigation ? "Hide from navigation" : "Show in navigation"}</button>
      {page.type !== "home" && <button type="button" onClick={async () => { const next = removeWebsitePage(document, page.id); if (next) await onSave(next); }} disabled={disabled} className={REMOVE_BUTTON}>Remove</button>}
...
export default function WebsitePageManager({ document, selectedPath, saving, onSelect, onSave, onAddEssentialPages }: { document: WebsiteSiteDocument; selectedPath: string; saving: boolean; onSelect: (path: string) => void; onSave: (document: WebsiteSiteDocument) => Promise<boolean>; onAddEssentialPages?: () => Promise<void> }) {
  ...
  return <section className="mb-5 rounded-2xl border border-cyan-400/20 bg-slate-900/85 p-5" aria-labelledby="website-page-manager-title">
    ...
    {removedPages.length > 0 && <div className="mt-5 border-t border-white/10 pt-4"><p className="text-xs font-semibold text-slate-400">Removed pages</p><ul className="mt-2 flex flex-wrap gap-2">{removedPages.map((page) => <li key={page.id} className="flex items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-300"><span>{page.title}</span><button type="button" disabled={saving} onClick={async () => { const next = restoreWebsitePage(document, page.id); if (next) await onSave(next); }} className={ACTION_BUTTON}>Restore hidden</button></li>)}</ul></div>}
  </section>;
}
