...
export async function GET(request: Request) {
  ...
  if (!absoluteWeeklyReportUrl("configuration-check")) return Response.json({ error: "Weekly report origin is not configured." }, { status: 503 });
  ...
  for (const project of allProjects) {
    ...
    if (!report) continue;
    const viewUrl = absoluteWeeklyReportUrl(project.id); if (!viewUrl) return Response.json({ error: "Weekly report origin is not configured." }, { status: 503 });
    const message = weeklyReportMessage(report, viewUrl);
