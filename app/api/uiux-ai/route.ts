...
const UIUX_AI_WEBHOOK =
  "https://rohitm2026.app.n8n.cloud/webhook/uiux-ai";
const UIUX_AI_WORKFLOW = "uiux-ai";
...
async function finalizeUsage(
  ...
  try {
    ...
    console.error("UI/UX AI usage finalization failed.");
    return false;
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("UI/UX AI project authorization failed.");
    return NextResponse.json(
  ...
  try {
    usageId = await startAiUsage({
      ...
      projectId,
      module: "uiux",
      workflow: UIUX_AI_WORKFLOW,
      model: null,
    });
  } catch (error) {
    if (error instanceof Response) return error;
    console.error("UI/UX AI usage initialization failed.");
    return NextResponse.json(
      { error: "Unable to track UI/UX AI request." },
      { status: 500 }
    );
  }

  const uiuxPayload = { ...body };
  delete uiuxPayload.projectId;
  const controller = new AbortController();
  const startedAt = Date.now();
  const timeout = setTimeout(() => controller.abort(), 120_000);

  try {
    const response = await fetch(UIUX_AI_WEBHOOK, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify(uiuxPayload),
    ...
    if (!text.trim()) {
      await finalizeUsage(usageId, "failed", startedAt);
      return NextResponse.json(
        { error: "UI/UX AI returned an empty response." },
        { status: 502 }
      );
    }
    ...
    if (n8nExecutionId) {
      try {
        ...
        console.error("UI/UX AI execution association failed.");
      }
    }
      ...
      await finalizeUsage(usageId, "failed", startedAt);
      return NextResponse.json(
        { error: "UI/UX AI returned invalid JSON." },
        { status: 502 }
      );
    }
  } catch {
    await finalizeUsage(usageId, "failed", startedAt);
    console.error("UI/UX AI request failed.");
