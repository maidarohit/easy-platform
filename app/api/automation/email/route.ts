...
async function finalizeUsage(usageId: string, status: "success" | "failed", startedAt: number, usageComponents?: readonly AiUsageComponent[]) {
  try {
    ...
    if (status === "success") await completeAiUsage({ usageId, durationMs, usageComponents });
    else await failAiUsage({ usageId, durationMs });
    return true;
  } catch {
    console.error("Email automation usage finalization failed.");
    return false;
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    usageId = await startAiUsage({ userId, projectId, module: "automation-email", workflow: WORKFLOW });
  } catch {
    console.error("Email automation usage initialization failed.");
    return Response.json({ error: "Unable to track email automation request." }, { status: 500 });
  }
  ...
  try {
    ...
    if (!n8nResponse.ok) {
      ...
      return Response.json({ error: "Email automation failed." }, { status: n8nResponse.status });
    }

    if (!responseText.trim()) {
      await finalizeUsage(usageId, "failed", startedAt);
      return Response.json({ error: "Email automation returned an empty response." }, { status: 502 });
    }
    ...
    try {
      ...
      await finalizeUsage(usageId, "failed", startedAt);
      return Response.json({ error: "Email automation returned an invalid response." }, { status: 502 });
    }
    ...
    if (n8nExecutionId) {
      try {
        ...
        console.error("Email automation execution association failed.");
      }
    }
    ...
    return Response.json({ error: "Email automation failed." }, { status: 500 });
  } finally {
