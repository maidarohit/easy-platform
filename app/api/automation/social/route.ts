...
const WORKFLOW = "automation-social";
...
async function finalizeUsage(
  ...
  try {
    ...
    console.error("Social automation usage finalization failed.");
    return false;
  }
}

export async function POST(request: Request) {
  const authorization = await authorizeAutomationRequest(request, "automation-social");
  if (!authorization.ok) return authorization.response;

  const { body, projectId, userId } = authorization;
  const webhook = getN8nWebhookConfig("N8N_AUTOMATION_SOCIAL_WEBHOOK_URL");
  ...
  try {
    usageId = await startAiUsage({
      userId,
      projectId,
      module: "automation-social",
      workflow: WORKFLOW,
    });
  } catch {
    console.error("Social automation usage initialization failed.");
    return Response.json(
      { error: "Unable to track social automation request." },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    if (!responseText.trim()) {
      await finalizeUsage(usageId, "failed", startedAt);
      return Response.json(
        { error: "Social automation returned an empty response." },
        { status: 502 }
      );
    }
    ...
    try {
      ...
      await finalizeUsage(usageId, "failed", startedAt);
      return Response.json(
        { error: "Social automation returned an invalid response." },
        { status: 502 }
      );
    }
    ...
    if (n8nExecutionId) {
      try {
        ...
        console.error("Social automation execution association failed.");
      }
    }
    ...
    await finalizeUsage(usageId, "failed", startedAt);
    console.error("Social automation request failed.");
    return Response.json(
      { error: "Social automation failed." },
      { status: 500 }
    );
